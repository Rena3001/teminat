...
@section('content')
<form action="{{ route('admin.products.store') }}" method="POST" class="row mb-3" enctype="multipart/form-data" style="row-gap: 20px;">
    @csrf
    <div class="col-lg-7">
                ...
                <div class="tab-content mt-3" id="myTabContent">
                    ...
                    <div class="tab-pane fade {{ $key === 0 ? 'active show' : '' }}" id="{{ 'slide' . $lang->code }}" role="tabpanel" aria-labelledby="{{ $lang->code . 'slide' }}">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('title.' . $lang->code)is-invalid @enderror" id="title_{{ $lang->code }}" name="title[{{ $lang->code }}]" value="{{ old('title.' . $lang->code) }}">
        ...
        <div class="card">
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input type="file" class="custom-file-pdf form-control @error('pdf_file') is-invalid @enderror" id="pdf_file" name="pdf_file" value="{{ old('pdf_file') }}" accept="application/pdf,application/vnd.ms-excel">
                    @error('pdf_file')
                    <span class="error invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group d-flex">
                    ...
                    <input type="file" name="image" class="custom-file-image form-control @error('image') is-invalid @enderror" id="image" value="{{ old('image') }}" accept="image/*">
                    @error('image')
                    <span class="error invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
