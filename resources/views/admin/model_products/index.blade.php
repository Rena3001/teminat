...
@endpush
@push('section_title')
Mehsulun Modelinin Siyahısı
@endpush

@section('content')
<div class="card mb-3">
    <div class="card-header p-4 border-bottom bg-body d-flex align-items-center justify-content-between my-3">
        <a class="btn btn-sm btn-phoenix-success" href="{{ route('admin.model_products.create') }}" role="button" aria-controls="basic-example-code">
            Yenisini əlavə edin
        </a>
        ...
        <div class="p-4 code-to-copy">
            <div id="tableExample3" data-list='{"valueNames":["id","title"],"page":5,"pagination":true}'>
                ...
                <div class="table-responsive">
                    {{-- <form id="bulk-delete-form" method="post" action="{{ route('admin.model_products.bulk-delete') }}">
                        @csrf
                        @method('delete') --}}
                                    ...
                                    <td class="align-middle white-space-nowrap text-end pe-0">
                                        <div class="btn-reveal-trigger position-static">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end py-2">
                                                <a class="dropdown-item" href="{{ route('admin.model_products.show', $model->id) }}"><span data-feather="info"></span></a>
                                                <a class="dropdown-item" href="{{ route('admin.model_products.edit', $model->id) }}"><span data-feather="edit"></span></a>
                                                <div class="dropdown-divider"></div>
                                                <form action="{{ route('admin.model_products.destroy', $model->id) }}" method="post" onclick="return confirm('Əminsiniz?','Bəli','Xeyir')">
                                                    @csrf
                                                    @method('delete')
    ...
    Sortable.create(tbody, {
        ...
        onEnd: function(evt) {
            ...
            for (let i = 0; i < items.length; i++) {
                ...
                model_products.push({ id: dataId, order: i + 1 });
            }

            fetch(`{{ url('api/model_products/changeOrder') }}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
                },
                body: JSON.stringify(model_products)
            })
