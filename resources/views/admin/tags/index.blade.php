...
<div class="card mb-3">
    <div class="card-header p-4 border-bottom bg-body d-flex align-items-center justify-content-between my-3">
        <a class="btn btn-sm btn-phoenix-success" href="{{ route('admin.tags.create') }}" role="button" aria-controls="basic-example-code">Yenisini əlavə edin</a>
    ...
    <div class="card-body">
        <div class="p-4 code-to-copy">
            <div id="tableExample3" data-list='{"valueNames":["order","id","slug","title"],"page":5,"pagination":true}'>
                            ...
                            <tbody class="list">
                                ...
                                <tr>
                                    <td class="fs-9 align-middle">
                                        <div class="form-check mb-0 fs-8">
                                            <input class="form-check-input" type="checkbox" name="selected_ids[]" value="{{ $tag->id }}">
                                        </div>
                                    </td>
                                    <td class="align-middle ps-3 id">{{ $tag->id }}</td>
                                    <td class="align-middle slug">{{ $tag->slug }}</td>
                                        ...
                                        <div class="btn-reveal-trigger position-static">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end py-2">
                                                <a class="dropdown-item" href="{{ route('admin.tags.show', $tag->id) }}">
                                                    <span data-feather="info"></span> Show
                                                </a>
                                                <a class="dropdown-item" href="{{ route('admin.tags.edit', $tag->id) }}">
                                                    <span data-feather="edit"></span> Edit
                                                </a>
                                                <div class="dropdown-divider"></div>
                                                <form action="{{ route('admin.tags.destroy', $tag->id) }}" method="post" onclick="return confirm('Əminsiniz?')">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="submit" class="dropdown-item text-danger"><span data-feather="delete"></span> Delete
                                                    </button>
                                                </form>
                                            </div>
                                        </div>
